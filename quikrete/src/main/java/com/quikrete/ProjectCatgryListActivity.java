...
import com.quikrete.adapter.ItemListAdapter_Product_Cat;
import com.quikrete.gsondata.projectcat.ProjectCategoryData;
import com.quikrete.gsondata.projectcat.Result;
...
public class ProjectCatgryListActivity extends Activity {
	...
	TextView txt_header_barcode, txt_header_mappoint, txt_header_search,
			txt_header_menu,txt_header,txt_header_favorites;
	...
	private ItemListAdapter_Product_Cat mAdapter;
	...
	// LinearLayout ll_header, ll_content;
	...
	private void listViewInit() {
		// TODO Auto-generated method stub
		mAdapter = new ItemListAdapter_Product_Cat(this,
		...
		mListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				intent.putExtra("cat_id", mAdapter.getItem(arg2).getId());
				startActivity(intent);
				
				Utils.getTracker(mAdapter.getItem(arg2).getName(), getApplication());
			}
		});
	}
	...
	protected void onResume() {
		...
		ll_header_up.post(new Runnable() {
			public void run() {
				...
				// int dimens_header[] = returnDimens(ll_header);
				// ScreenUtils.HEADER_INIT_Y = dimens_header[1];
				// ScreenUtils.HEADER_HEIGHT = dimens_header[3];
	...
	class AsyncLoadProductCategory extends
		...
		protected List<Result> doInBackground(String... params) {
			...
			try {
				...
				InputStream source = WebServiceUtil.retrieveStream(
						WebServiceUtil.URL
								+ WebServiceUtil.METHOD_2_GET_PROJECT_CAT_LIST,
						context);
				Gson gson = new Gson();
				Reader reader = new InputStreamReader(source);
				ProjectCategoryData response = gson.fromJson(reader,
						ProjectCategoryData.class);
		...
		protected void onPostExecute(List<Result> reultantBanks) {
			...
			if (reultantBanks != null) {
				Log.e("newlog", reultantBanks.size() + "---size");
				for (Result result : reultantBanks) {
	...
	private void liftUpDownMenu(LinearLayout ll_header,
			LinearLayout ll_content, int startY1, int endY1, int startY2,
			int endY2) {

		returnAnimation(ll_header, startY1, endY1);
		// returnAnimation(ll_content, startY2, endY2);

	}

	private ObjectAnimator returnAnimation(LinearLayout ll, int startY, int endY) {

		ObjectAnimator objectAnimator = ObjectAnimator.ofFloat(ll,
	...
	/** {X,Y,WIDTH,HEIGHT} */
	private int[] returnDimens(LinearLayout ll) {

		Rect rectf = new Rect();
		ll.getLocalVisibleRect(rectf);

		int location[] = { 0, 0 };
		ll.getLocationOnScreen(location);

		int dimen[] = { rectf.width(), rectf.height() };
