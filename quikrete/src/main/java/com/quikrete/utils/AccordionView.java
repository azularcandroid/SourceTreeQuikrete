...
public class AccordionView extends LinearLayout {
	...
	public AccordionView(Context context, AttributeSet attrs) {
		...
		if (attrs != null) {
			...
			int sectionVisibilityResourceId = a.getResourceId(
					R.styleable.accordion_section_visibility, 0);
			...
			if (sectionVisibilityResourceId != 0) {
				sectionVisibilities = getResources().getIntArray(
						sectionVisibilityResourceId);
			}
		}
	...
	private View getView(final LayoutInflater inflater, int i, boolean hide) {
		...
		// FontUtils.setCustomFont(container,
		// AccordionView.this.getContext().getAssets());
	...
	private View getViewHeader(LayoutInflater inflater, final int position,
		...
		final TextView fontIcon = (TextView) view
				.findViewById(R.id.txt_font_icons);

		fontIcon.setTypeface(ScreenUtils.returnTypeFace(context));
		...
		view.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (wrappedChildren[position].getVisibility() == VISIBLE) {
					((TextView) view.findViewById(headerLabel))
							.setTextColor(Color.parseColor("#423F3E"));
					fontIcon.setText(context.getString(R.string.fa_angle_down));
				} else {
					((TextView) view.findViewById(headerLabel))
							.setTextColor(Color.parseColor("#878685"));
					fontIcon.setText(context.getString(R.string.fa_angle_right));
				}
	...
	protected void onFinishInflate() {
		...
		for (int i = 0; i < childCount; i++) {
			final boolean hide = sectionVisibilities.length > 0
					&& sectionVisibilities[i] == 0;
	...
	public void setSectionVisibility(int position, int visibility) {
		...
		if (headerFoldButton != 0) {
			final View foldButton = headers[position]
					.findViewById(headerFoldButton);
			if (foldButton instanceof ToggleImageLabeledButton) {
				final ToggleImageLabeledButton toggleButton = (ToggleImageLabeledButton) foldButton;
				toggleButton
						.setState(wrappedChildren[position].getVisibility() == VISIBLE);
	...
	public void setSectionVisibility(int postion){
		sectionContainers[postion].setVisibility(View.GONE);
	}
